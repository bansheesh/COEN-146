...
* Title: Lab 6 - UDP server with packets that have checksum, sequence number, acknowledgement number, and timer
* Description: This program implements a server that receives packets from a client,
* checks the checksum, sequence number, and sends acknowledgments.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
Packet serverReceive(int sockfd, struct sockaddr *address, socklen_t *addrlen, int seqnum) {
    ...
    while (1) {
	    ...
	    int n = recvfrom(sockfd, &packet, sizeof(Packet), 0, address, addrlen);
        if (n < 0) {
...
int main(int argc, char *argv[]) {
    ...
    int fp = open(argv[2], O_WRONLY | O_CREAT | O_TRUNC, 0666);
    if(fp == -1){
