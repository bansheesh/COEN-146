...
* Title: Lab 6 - UDP client with packets that have checksum, sequence number, acknowledgement number, and timer
* Description: This program implements a UDP based client for file transfer, where packets are sent to the server
* with checksum, sequence numbers, and a retry mechanism for lost or corrupted packets. 
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void clientSend(int sockfd, const struct sockaddr *address, socklen_t addrlen, Packet packet, unsigned retries) {
    while (1) {	
        ...
        //so, there is an ACK to receive
            ...
            //check the checksum. if bad checksum, resend packet
            if (recvpacket.header.cksum != e_cksum) {
                printf("Client: Bad checksum, expected checksum was: %d\n", e_cksum);
            //check the sequence number. if incorrect sequence number, resend packet
